...
float Math::roundDown(float x, float multiple)
{
	return (int)(x / multiple - (x < 0 ? ((int)(x / multiple) != multiple ? 1 : 0) : 0)) * multiple;
}

float Math::cosineInterpolation(float x, float y, float blend)
{
	float f = (float)(1 - cos(blend * PI_F)) * 0.5f;
	return x * (1.0f - f) + y * f;
}

void Math::clamp(float& x, float low, float high)
{
	if (x < low)
		x = low;
	if (x > high)
...
glm::mat4 Math::createProjectionMatrix(float nearPlane, float farPlane, float fov)
{
...
glm::mat4 Math::createModelMatrix(const glm::vec3& translate, const glm::vec3& rotate, float scale)
{
	glm::mat4 matrix(1.0);
...
glm::vec3 Math::getBarycentricCoords(const glm::vec2& p, const glm::vec2& a, const glm::vec2& b, const glm::vec2& c)
{
...
float Math::getBarycentricHeight(const glm::vec3& p1, const glm::vec3& p2, const glm::vec3& p3, const glm::vec2& pos) {
